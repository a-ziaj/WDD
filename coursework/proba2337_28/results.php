...
<head>
    ...
    <style>
        ...
        .subset-form { background: #f5f5f5; padding: 15px; margin-bottom: 20px; }
...
<body>
    ...
    <?php if (isset($subset_error)): ?>
        <div class="error-msg"><?= htmlspecialchars($subset_error) ?></div>
    <?php elseif (isset($subset_success)): ?>
        <div class="success-msg">Successfully created subset with <?= htmlspecialchars($_POST['num_sequences']) ?> sequences</div>
    <?php endif; ?>

    <div class="subset-form">
    ...
    <div>
        <h3>Amino Acid Content Analysis</h3>
        <form action="content.php" method="get">
            <input type="hidden" name="job_id" value="<?= htmlspecialchars($job_id) ?>">
            <?php if (isset($current_subset)): ?>
                <input type="hidden" name="subset" value="<?= htmlspecialchars($_POST['num_sequences']) ?>">
            <?php endif; ?>
            <input type="submit" value="Analyze AA Content">
        </form>
    </div>
</body>
</html>
